...
Page({
  data: {
    ...
    nav_go:'/pages/index/index',
    nav_img:'/images/1.png',
    nav_img_title:'首页',
    nav_type:'navigate'
  },
  ...
  onShow() {
    // this.setData({ nav_title: (this.data.goods == null) ? app.data.common_pages_title.goods_detail : this.data.goods.title});
  },
  // 获取数据列表
  init() {
    // 数据初始化
      ...
      wx.request({
        ...
        success: res => {
          ...
          if (res.data.code == 0) {
            ...
            data.goods.sp_price = app._split(data.goods.price)
            ...
            self.setData({
              ...
              // goods_spec_base_price: obj.price,
              goods_spec_base_original_price: data.goods.original_price,
              // goods_spec_base_original_price: obj.price,
            ...
            let par_data = this.data.params || ''
            //有加盟商id 就有进店，0 是自营产品
            if (par_data.type == '1'){
              if ((par_data.bmid || null) != null) {
                this.setData({
                  nav_go: '/pages/brand-merchant/brand-merchant?brand_merchant_id=' + par_data.bmid || data.goods.brand_merchant_id,
                  nav_img: '/images/shangjia.png',
                  nav_img_title: '总店'
                })
              }
            } else if ((data.goods.brand_merchant_id || null) != null && data.goods.brand_merchant_id != '0'){
              this.setData({
                nav_go: '/pages/brand-merchant/brand-merchant?brand_merchant_id=' +  data.goods.brand_merchant_id,
                nav_img: '/images/shangjia.png',
                nav_img_title: '总店'
              })
            }
            // let is_inn = (pages.length > 8 ? true : false)
            // //
            // if (is_inn) { this.setData({ nav_type:'reLaunch'})}
            
            console.log("栈栈栈", pages)
            //类型为 1 表示是门店页面进来的 
            if (par_data.type == '2'){
              if ((par_data.shopid || null) != null) {
                this.setData({
                  nav_go: '/pages/shop/shop?shop_id=' + par_data.shopid || data.goods.shop_id,
                  nav_img: '/images/shangjia.png',
                  nav_img_title: '门店'
                })
              }
            ...
            if (par_data.type == "3") {
              if ((par_data.stid || null) != null) {
                this.setData({
                  nav_go: '/pages/station/station?station_id=' + par_data.stid || 0,
                  nav_img: '/images/shangjia.png',
                  nav_img_title: '加盟点'
                })
              }
  ...
  cart_submit_event(e) {
    ...
    if (this.data.nav_submit_is_disabled) return
  ...
  goods_cart_event(e, spec) {
    ...
    if (app.user_is_need_login(user)) {
      ...
      wx.request({
        ...
        method: 'POST',
        data: { "goods_id": this.data.goods.id, "stock": this.data.temp_buy_number, "spec": JSON.stringify(spec), 'shop_id': this.data.shopInfo.id},
        dataType: 'json',
        success: (res) => {
          wx.hideLoading();
          if (res.data.code == 0) {
  ...
  goods_buy_confirm_event(e) {
    ...
    if (app.user_is_need_login(user)) {
      ...
      let e = this.data.shopInfo.id || this.data.shopInfo
      ...
      switch (this.data.buy_event_type) {
        case 'buy' :
          ...
          var data = {
            ...
            'shop_id': this.data.shopInfo.id || 0 ,//商铺id
            "brand_merchant_id": this.data.goods.brand_merchant_id,
          };
          wx.navigateTo({
  ...
  poster_event() {
    ...
    if (app.user_is_need_login(user)) {
      ...
      wx.request({
        url: app.get_request_url('poster', 'goods'),//api/index/poster
        method: 'POST',
